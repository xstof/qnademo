...
# Create context for storage account:
$Key = Get-AzStorageAccountKey -ResourceGroupName $RG -Name $StorageAccountName
$StorageContext = New-AzStorageContext -StorageAccountKey $Key[0].Value -StorageAccountName $StorageAccountName

# Enable the storage account for static websites
...
Get-ChildItem -File -Recurse $PathForFilesToUpload | ForEach-Object { 
  ...
  if([String]::IsNullOrWhiteSpace($ContentType)){
    ...
    Set-AzStorageBlobContent -File $_.FullName -Blob $BlobName -Container `$web -Context $StorageContext -Properties @{"ContentType" = $ContentType} -Force
  }
